...
La compilation fonctionne (nous avons commenté la ligne d'appel à la fonction afin d'éviter une erreur et de voir le message affiché), l'exécution affiche `Le programme compile sans MON_FILTRE defini`. C'est la preuve que la fonction n'est pas appliquée.

Une autre preuve (bonus) : Lorsque nous décommentons l'appel à la fonction, l'éditeur de code nous signale directement, et la compilation le confirme :
...
Ici, nous allons rendre la condition vraie : nous allons remplacer `Linux` par `Windows` dans l'instruction `with filter()` :
...
Cette fois, la fonction est bien présente et appelable. Le filtre qui la rend visible est valable, et donc le compilateur y a accès normalement. Nous avons laissé le dépôt sur l'état où la condition est fausse (`with filter("system:Linux")`)
